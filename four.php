					...
					<?php if (isset($tblqiymet) && !empty($tblqiymet)): ?>
						<table class="table table-bordered table-primary table-striped table-hover">
							...
							<?php if(!isset($_SESSION['student']) && !isset($_SESSION['fenler'])):?>
								<tr>
						      		<th>Şagirdlər</th>
								<?php foreach ($tbltarix2 as $history):  ?>		 
			     					<th colspan="<?php echo count($fenns2);?>"><?php echo date("m-d-Y", strtotime($history)); ?></th>		
					      		<?php endforeach;?>
					      			<th colspan="<?php echo count($fenns2);?>"><?php echo "ortalama";?></th>	
						    	</tr>
						    	<tr>
						    		<td></td>
						    		<?php foreach ($tbltarix2 as $history2):  ?>
					    				<?php for($ty=0; $ty<count($fenns3); $ty++): ?>	
											<td><?php echo $fenns3[$ty];?></td>
										<?php endfor;?>
										<?php 
											for ($dx=0; $dx < count($table); $dx++) {
												if ($history2 == $table[$dx]['create_date']) {	 
													foreach ($tblad as $name_name) { 
														$yox = 0;
														if ($name_name==$tblname1[$dx]) {
															for ($jk=0; $jk<count($fenns3);$jk++) {
																if ($fenns3[$jk] == $fenns1[$dx]) {
																	$final[] = array($name_name,$tblqiymet[$dx]);
																	$yox = 1;
																}
																if ($jk == count($fenns3)-1) {
																	if ($yox == 0) {

																		$final[] = array($name_name,"---");
								...
								<?php foreach ($tblad as $tabname): ?>
							  		<tr>
							  			<td class="sagirdTd"><?php echo $tabname; ?></td>
							  			<?php for($mx=0;$mx < count($final);$mx++):?>
							  				<?php if($tabname==$final[$mx][0]):?>
							  					<td><?php echo $final[$mx][1];?></td>
							  				<?php endif;?>
							  			<?php endfor?>
									</tr>
